class Reservation < ApplicationRecord
  ...
  def date_three_month_end
    errors.add(:day, "は3ヶ月以降の日付は選択できません") if (Date.current >> MAX_MONTHS) < day
  end

  def self.reservations_after_three_month
    # 今日から3ヶ月先までのデータを取得
    reservations = Reservation.all.where("day >= ?", Date.current).where("day < ?", Date.current >> MAX_MONTHS).order(day: :desc)
    # 配列を作成し、データを格納
    # DBアクセスを減らすために必要なデータを配列にデータを入れています
    reservation_data = []
  ...
  def self.reservation_date
    where("day < ?", Date.current >> MAX_MONTHS)
  end
end
